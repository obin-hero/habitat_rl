...
from model import *  #TODO: import your policy in here or model/__init__.py
import time
TIME_DEBUG = False
ADD_IL = False
...
class PPOTrainer_Memory(BaseRLTrainer):
    ...
    def _setup_actor_critic_agent(self, ppo_cfg: Config) -> None:
        ...
        if ppo_cfg.pretrained_encoder or ppo_cfg.pretrained or ppo_cfg.il_pretrained:
            pretrained_state = torch.load(ppo_cfg.pretrained_weights, map_location="cpu")

        #TODO: edit this code to load your ckpt model
    ...
    def exchange_lasts(self):
        saved_last_obs = self.last_observations
        saved_last_recurrent_hidden = self.last_recurrent_hidden_states
        saved_last_prev_acts = self.last_prev_actions
        saved_last_masks = self.last_masks

        self.last_observations = self.saved_last_obs
        self.last_recurrent_hidden_states = self.saved_last_recurrent_hidden_states
        self.last_prev_actions = self.saved_last_prev_actions
        self.last_masks = self.saved_last_masks

        self.saved_last_obs = saved_last_obs
        self.saved_last_recurrent_hidden_states = saved_last_recurrent_hidden
        self.saved_last_prev_actions = saved_last_prev_acts
        self.saved_last_masks = saved_last_masks
    ...
    def _update_agent(self, ppo_cfg, rollouts, rollouts2=None):
        ...
        value_loss, action_loss, dist_entropy, il_loss, unexp_loss, targ_loss = self.agent.update(rollouts, rollouts2)

        rollouts.after_update()
        if rollouts2 is not None : rollouts2.after_update()
    ...
    def train(self) -> None:
        ...
        if ADD_IL:
            self.il_envs = construct_envs(
                self.config, eval(self.config.ENV_NAME), no_val=True
            )
        ...
        for sensor in rollouts.observations:
            try:
                ...
                print('i')
        self.last_observations = batch
        self.last_recurrent_hidden_states = torch.zeros(self.actor_critic.net.num_recurrent_layers, total_processes,
        ...
        if ADD_IL:
            ...
            for sensor in rollouts2.observations:
                rollouts2.observations[sensor][0].copy_(batch2[sensor][:num_train_processes])
            self.saved_last_obs = batch2
            self.saved_last_recurrent_hidden_states = torch.zeros(self.actor_critic.net.num_recurrent_layers,
                                                                  total_processes, ppo_cfg.hidden_size).to(self.device)
            self.saved_last_prev_actions = torch.zeros(total_processes, rollouts2.prev_actions.shape[-1]).to(self.device)
            self.saved_last_masks = torch.zeros(total_processes, 1).to(self.device)
            batch2 = None
        else:
            rollouts2 = None
        ...
        with TensorboardWriter(
            ...
            for update in range(self.config.NUM_UPDATES):
                ...
                for collect_mode in modes:
                    ...
                    for step in range(ppo_cfg.num_steps):
                        ...
                        (
                            ...
                            use_rollouts, current_episode_reward, running_episode_stats
                        )
                ...
                (
                    ...
                    dist_entropy,
                    otherlosses
                ) = self._update_agent(ppo_cfg, rollouts, rollouts2)
    ...
    def _eval_checkpoint(
        ...
        while (
            ...
            batch, rewards, dones, infos = self.envs.step([a[0].item() for a in actions])
    ...
    def benchmark(self) -> None:
        ...
        if ADD_IL:
            ...
            for sensor in rollouts2.observations:
                rollouts2.observations[sensor][0].copy_(batch2[sensor][:num_train_processes])
            self.saved_last_obs = batch2
            self.saved_last_recurrent_hidden_states = torch.zeros(self.actor_critic.net.num_recurrent_layers, total_processes, ppo_cfg.hidden_size).to(self.device)
            self.saved_last_prev_actions = torch.zeros(total_processes, rollouts2.prev_actions.shape[-1]).to(self.device)
            self.saved_last_masks = torch.zeros(total_processes,1).to(self.device)
            batch2 = None
        else:
